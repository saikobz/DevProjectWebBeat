-- Generated from database/schema.sql — do not edit by hand.
...
create index beats_status_published_at_idx on public.beats(status, published_at desc);
create index beats_genre_idx on public.beats(genre);
create index beats_mood_gin_idx on public.beats using gin(mood);
create index beats_tags_gin_idx on public.beats using gin(tags);
